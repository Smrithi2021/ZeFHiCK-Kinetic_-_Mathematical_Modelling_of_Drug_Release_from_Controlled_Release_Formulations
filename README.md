...
Theory
The drug releases from controlled release formulations have varied physical properties that influence their release. The release patterns can be zero order, first order, and initial rapid followed by slow zero order etc. The use of mathematical models enables the prediction of release kinetics before the release systems are realized. It allows the measurement of some important physical parameters, such as the drug diffusion coefficient and resort to model fitting on experimental release data.
	The methods of approach to investigate the kinetics of drug release from controlled release formulation can be classified as:
	Statistical method 
	Model dependent method 
	Model independent method
Statistical method
Statistics means the collection, analysis, interpretation, presentation, and organization of data. Various approaches based on statistical calculation such as:
	Exploratory Data Analysis methods - This method can be used in the first step to compare dissolution profile data in both graphical and numerical manner. The dissolution profile data are illustrated graphically by plotting the mean dissolution profile data for each formulation with error bars extending to two standard errors at each dissolution time point. Then, the data of the dissolution profiles are summarized numerically and 95% confidence intervals for the differences in the mean dissolution profiles at each dissolution time point are evaluated
	Multivariate approach (MANOVA) - These methods were based upon repeated measures designs where time is the repeated factor and percent dissolved is the dependent variable.
Model dependent method
